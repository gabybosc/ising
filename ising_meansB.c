...
int main(){
	...
	int paso = 6; // paso*N*N pasos entre iteraciones guardadas
	
	// media sobre n_tira con termalizacion para cada temp
	int n_tira = 30000;
	int termalizacion = 8000/(N*N) + 1; //porque flipear avanza de a N*N pasos
	...
	FILE *fp;
	char fn[50];
	sprintf(fn,"meansB_corr.txt");
	fp = fopen(fn, "w");
	
	fprintf(fp, "term %d, %d pasos de %d\n",termalizacion,n_tira,paso*N*N);
	...
	B = 1.0;
	J = 0.0;
	for (temp=temp_f; temp>temp_i; temp-=temp_paso){//loop temperatura
		*magnetizacion = 0;
		for (l = 0; l < N*N; l++){
			*magnetizacion += *(red+l);
