...
for i,valor in enumerate(BJ):
	if i%2: # si no quiero graficar todos
		plt.plot(abs(datos.iloc[i,1:]),label='B={}'.format(valor))
plt.xlabel(r'distancia k ($\times1000$)')
plt.ylabel(r'correlacion media $\langle C(k) \rangle$')
plt.legend()
#plt.xlim([0,10])
plt.grid()
plt.savefig('img/correlacionB.png',dpi=150)
plt.show()
