...
int main(){
	...
	int k_max = 350; // Esto NO es k, es k per site: (k/(N*N))
	...
	B=0;
	poblar(red);
	for (J=0.1; J<0.52; J+=0.1){//inicio loop J		
	...
	for (J=0.52; J<0.61; J+=0.01){//inicio loop J
		//SE PODRIA MODIFICAR TERMALIZACION PARA CADA J
		
		for (k=0; k<k_max; k++){
			*(correlacion+k) = 0;
		}//set correlacion a 0 para cada J
